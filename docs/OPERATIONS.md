...
| `/actuator/health` | exposed | exposed | exposed | no | Protected aggregate; production hides details. |
...
The current security model has no separate infrastructure operator authority. Consequently only the two exact probe endpoints are anonymous; other exposed endpoints require the existing admin authority and should additionally be network-restricted. Management stays on the application port because the current Compose/frontend topology does not safely route a second port.
...
A PostgreSQL outage makes readiness `DOWN` while liveness remains `UP`; drain the instance and investigate DB reachability/pool metrics rather than restarting a healthy JVM repeatedly. Gemini or SMTP outages do not change either probe. They surface when the optional operation is used and are categorized in application logs. Health checks never generate content or send mail.
...
* `event=HTTP_SLOW_REQUEST` is the single WARN completion event for requests over `SLOW_REQUEST_THRESHOLD`. Faster successes are DEBUG, failures are WARN, and health/metrics/Swagger traffic is excluded from access logging. Raw query strings and bodies are never included.
...
* Unexpected exceptions have one authoritative ERROR stack trace in `GlobalExceptionHandler`. Expected validation/domain failures remain safe responses without ERROR stack traces. Application logs are not the durable audit trail.
