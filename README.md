...
Start PostgreSQL; Spring Boot Flyway applies and validates `backend/src/main/resources/db/migration` automatically, then run:
...
Spring Security uses an `IF_REQUIRED` session only to retain the OAuth authorization request and validate `state` during the Google round trip. SouthRail does not persist an authenticated `SecurityContext` in that session, invalidates the temporary session after every OAuth outcome, and continues to authorize APIs exclusively with SouthRail JWTs. Used and expired one-time exchange-code rows are retained briefly for audit/replay safety and automatically deleted daily after a 24-hour retention period; override the cleanup schedule with `app.oauth.cleanup-cron` if required.
