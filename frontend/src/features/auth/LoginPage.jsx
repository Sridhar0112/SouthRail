...
import {
  ...
  TextField, Typography, Dialog, DialogContent, DialogActions
} from '@mui/material';
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 8 }}>
      <Paper sx={{ p: { xs: 3, sm: 5 } }}>
        ...
        {location.state?.message && <Alert severity="info" sx={{ mb: 2 }}>{location.state.message}</Alert>}
        {apiError &&
          !(
            (apiError?.message || '').toLowerCase().includes('account locked') ||
            (apiError?.message || '').toLowerCase().includes('temporarily locked')
          ) && (
            <Alert severity="error" sx={{ mb: 2 }}>
              {apiError?.message || apiError}
            </Alert>
          )}
        {unlockMessage && <Alert severity="success" sx={{ mb: 2 }}>{unlockMessage}</Alert>}
          ...
          <Stack spacing={2}>
            ...
            {showResend && (
              <Typography align="center" sx={{ mb: 2 }}>
                ...
                <Button variant="text" size="small" onClick={resendVerification}
                  disabled={resending} sx={{ textTransform: 'none', fontWeight: 700 }}>
      ...
      >
        ...
        <Box sx={{
          ...
          <Box sx={{ flex: 1, zIndex: 1 }}>
            <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '1.05rem', lineHeight: 1.25 }}>
              Account locked
            </Typography>
            <Typography sx={{ color: 'rgba(255,255,255,0.72)', fontSize: '0.73rem', mt: 0.25 }}>
          ...
          {/* Countdown badge */}
          <Box sx={{
            bgcolor: 'rgba(0,0,0,0.22)', borderRadius: '12px',
            px: 1.4, py: 0.6, zIndex: 1, textAlign: 'center', minWidth: 56
          }}>
        ...
        <DialogContent sx={{ px: 3, pt: 2.5, pb: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1.5, mb: 2 }}>
            <Box sx={{
              flex: 1, bgcolor: 'action.hover', borderRadius: '12px',
              px: 1.5, py: 1.25, display: 'flex', alignItems: 'center', gap: 1
            }}>
        ...
        <DialogActions sx={{ px: 3, pb: 3, pt: 1.5, gap: 1.5 }}>
          <Button
            ...
            sx={{ flex: 1, textTransform: 'none', borderRadius: '50px', fontWeight: 600, py: 1, fontSize: '0.82rem' }}
          >
            close
          </Button>
          <Button
            variant="contained" onClick={sendUnlockEmail}
